...
Extending the `IRouteBuilder` intrerface to map custom route is not supported anymore. Instead, use mapping attributes on composition handlers. See below for details about the composition handlers API.
