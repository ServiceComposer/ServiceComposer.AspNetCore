...
The only supported `Preview` overload of the `IViewModelPreviewHandler` is the one accepting an `HttpRequest`:

<!-- snippet: viewmodel-preview-handler-api -->
<a id='snippet-viewmodel-preview-handler-api'></a>
```cs
...
```
<sup><a href='/src/Snippets/UpgradeGuides/1.x-to-2.0/UpgradeGuide.cs#L77-L85' title='Snippet source file'>snippet source</a> | <a href='#snippet-viewmodel-preview-handler-api' title='Start of snippet'>anchor</a></sup>
<!-- endSnippet -->

The remaining two overloads have now a default implementation throwing a `NotSupportedException`.
