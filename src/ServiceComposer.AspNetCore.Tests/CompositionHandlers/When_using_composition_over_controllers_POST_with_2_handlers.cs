...
namespace ServiceComposer.AspNetCore.Tests.CompositionHandlers
{
    ...
    {
        [Route("/api/CompositionOverControllerPostUsingCompositionHandlers")]
    ...
    public class When_using_composition_over_controllers_POST_with_2_handlers
    {
        ...
        {
            [HttpPost("/api/CompositionOverControllerPostUsingCompositionHandlers/{id}")]
            public async Task Handle(HttpRequest request)
            {
                request.Body.Position = 0;
                using var reader = new StreamReader(request.Body, Encoding.UTF8, leaveOpen: true);
        ...
        public async Task Returns_expected_response()
        {
            ...
            // Act
            var response = await client.PostAsync("/api/CompositionOverControllerPostUsingCompositionHandlers/1", stringContent);
